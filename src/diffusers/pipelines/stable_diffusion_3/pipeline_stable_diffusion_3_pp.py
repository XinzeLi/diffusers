...
from yfusers.image_processor import VaeImageProcessor
from yfusers.loaders import FromSingleFileMixin, SD3LoraLoaderMixin
# from diffusers.models.autoencoders import AutoencoderKL
from yfusers.models.autoencoders.autoencoder_kl import AutoencoderKL
from yfusers.models.transformers.transformer_sd3 import SD3Transformer2DModel
from yfusers.schedulers import FlowMatchEulerDiscreteScheduler
from yfusers.utils import (
    USE_PEFT_BACKEND,
...
from yfusers.parallel_state import (
    get_cfg_group,
...
from yfusers.runtime_state import(
    get_runtime_state,
)
...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
        ...
        # xinze: every time updates a dict
        ...
        print(f"the config is {self.transformer.config}")
        # self.patch_size = (
    ...
    def _init_pipeline(self, latents: torch.Tensor):
        return latents
    ...
    def __call__(
        ...
        # xinze: here need to change!
        # print(f"the shape of embed prompt is {prompt_embeds.shape}, the shape of pooled_prompt_embeds is {pooled_prompt_embeds.shape}")
        ...
        # print(f"after split batch, the shape of embed prompt is {prompt_embeds.shape}, the shape of pooled_prompt_embeds is {pooled_prompt_embeds.shape}")

        # 4. Prepare timesteps
        timesteps, num_inference_steps = retrieve_timesteps(self.scheduler, num_inference_steps, device, timesteps)
        num_warmup_steps = max(len(timesteps) - num_inference_steps * self.scheduler.order, 0)
        self._num_timesteps = len(timesteps)
        # print(f"the timesteps is {timesteps}")
    ...
    def _sync_pipeline(
        ...
        for i, t in enumerate(timesteps):
            ...
            if not is_pipeline_first_stage():
                ...
                encoder_hidden_states = self.post_office.recv_shipment().content["encoder_hidden_states"]
            elif not i == 0:
                latents = self.post_office.recv_shipment().content["hidden_states"]
            ...
            print(f"the shape of latents is {latents.shape}, encoder hidden states is {encoder_hidden_states.shape}")
        ...
        for i, t in enumerate(timesteps):
            for patch_idx in range(num_pipeline_patch):
                ...
                if is_pipeline_last_stage():
                    ...
                    patch_latents[patch_idx] = self.scheduler.step(patch_latents[patch_idx], \
                                            t, last_patch_latents[patch_idx], return_dict=False)[0]
                ...
                if not is_pipeline_last_stage() and patch_idx == 0:
            ...
            if XLA_AVAILABLE and is_pipeline_last_stage():
                xm.mark_step()
